...
public class PersonController {
	...
	public List<PersonGetResponsePayload> getAll() {
		return readUseCase.readAll().stream().map(d -> mapper.toPersonGetResponsePayload(d))
				.collect(Collectors.toList());
	}

	@GetMapping(value = "/{id}")
	public PersonGetResponsePayload getById(@PathVariable(value = "id") String id) {
		return mapper.toPersonGetResponsePayload(readUseCase.readById(id));
	}

	@PostMapping
	@CrossOrigin(exposedHeaders = { HttpHeaders.LOCATION })
	public ResponseEntity<Void> post(@RequestBody PersonPostRequestPayload payload) {
	...
	public void patch(@PathVariable(value = "id") String id, @RequestBody PersonPatchRequestPayload payload) {
